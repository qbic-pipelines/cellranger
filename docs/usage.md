# nf-core/cellranger: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/cellranger/usage](https://nf-co.re/cellranger/usage)
...
```console
nextflow run nf-core/cellranger --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/cellranger
```
...
First, go to the [nf-core/cellranger releases page](https://github.com/nf-core/cellranger/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
